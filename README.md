...
Workflow Manager 1.0.73 是当前 Codex 的 Hard 任务安全层：普通开发保持原生体验，只在高风险、跨范围或未知根因任务中介入。

- **确认后执行**：Hard 任务先只读分析并生成可审阅计划；待确认计划接受明确执行表达或简短上下文同意，同时拒绝否定、条件、疑问、引用和范围变更。
- **可信授权**：同时校验子任务请求、宿主接受和实际启动，避免错误模型、错任务或伪造状态获得写权限。
- **原生表达**：计划、执行结果和复核使用普通有界文本；时钟、固定 marker、关键词、结尾和 JSON fence 不构成授权门禁。
- **准确路由**：明确“不得修改、测试、发布或 Git 写入”的只读元数据请求保持原生；同一请求里真实的生产发布、不可逆或跨范围证据仍会进入 Hard 判断。
- **安全重新分类**：对已写入但未确认的 Hard 计划，可用单行 `降级复核：<完整当前目标>` 请求独立重新分类，例如 `降级复核：仅修改 AndroidNativeDemo App 源码中的横屏布局`；仅实际属于 Simple 且无活动写入者时退休旧授权，原审计 journal 保留。仅作为检索线索的旧会话标题不会触发参考一致性 Hard。
...
py -3 -B "$CodexHome\plugins\cache\workflow-manager\workflow-manager\1.0.73\scripts\install_stable_skill.py" --codex-home "$CodexHome"
```
...
python3 -B "$codex_home/plugins/cache/workflow-manager/workflow-manager/1.0.73/scripts/install_stable_skill.py" --codex-home "$codex_home"
```
...
```powershell
$CodexHome = Join-Path $env:USERPROFILE ".codex"
py -3 -B "$CodexHome\plugins\cache\workflow-manager\workflow-manager\1.0.73\scripts\hook_trust_doctor.py" --cwd "C:\path\to\workspace"
```
...
```bash
codex_home="${CODEX_HOME:-$HOME/.codex}"
python3 -B "$codex_home/plugins/cache/workflow-manager/workflow-manager/1.0.73/scripts/hook_trust_doctor.py" --cwd /path/to/workspace
```
...
```bash
codex plugin marketplace add LOLer001/workflow-manager --ref v1.0.73 --json
```
