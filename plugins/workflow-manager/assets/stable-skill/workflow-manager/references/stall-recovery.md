...
The Hook binds the stall to the current objective, plan digest, execution contract, executor attempt, and failure kind. Wrong caller/status/contract/type, embedded or duplicate markers, or a second stall on the same contract exhaust the route instead of opening a loop.
...
The assessor ends with one exact line containing the current stall/binding/plan/contract and `outcome=resume|replan` plus a remediation digest. It remains read-only:
...
- `replan` means scope, ordering, risk, or acceptance must change. Invalidate the old plan/executor contract and return to detailed Hard planning plus strict user confirmation.
