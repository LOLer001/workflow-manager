...
This reference applies only after a Hard plan is confirmed. Schema 33/writer 1.0.64 uses execution profile v12 and an append-only canonical journal v3. A new objective owns an independent task epoch and journal; a worktree-only migration never clears its contract.
