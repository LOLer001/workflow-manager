...
- A named historical conversation used only as a lookup source does not request reference-fidelity acceptance. A direct instruction to match a reference still does.
...
- A single-line `降级复核：<完整当前目标>` requests an independent Simple classification only while a valid Hard plan is awaiting confirmation and no writer is pending, live, or unknown. A successful review retires old authority without deleting its journal; an incomplete scope, confirmed plan, or still-Hard objective stays protected.
