...
class SkillIdentityTests(unittest.TestCase):
    ...
    def test_skill_is_lean_and_leaves_native_judgment_to_codex(self) -> None:
        ...
        self.assertIn("do not call an assessor or ask for plan confirmation", self.work_routing)
        for retired in (
            "ends with exactly",
    ...
    def test_protocol_continuity_and_privacy_are_preserved(self) -> None:
        self.assertIn("Schema 33/writer 1.0.62", self.confirmed_execution)
        self.assertIn("execution profile v12", self.confirmed_execution)
    ...
    def test_release_docs_target_current_version_without_fixed_suite_counts(self) -> None:
        ...
        self.assertEqual(self.manifest["version"], "1.0.62")
        self.assertIn("/1.0.62/", readme)
