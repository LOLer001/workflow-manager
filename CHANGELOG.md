...
- 新增显式单行 `降级复核：<完整当前目标>`：只在 Hard 计划已写入但尚未确认、没有执行或状态未知的写入者、且 journal 完整时，独立判定重述目标是否为 Simple。成功后原子退休旧授权、保留原 journal 审计字节，并清除旧参考验收、确认回执和写入权限；笼统要求降级或仍属 Hard 的目标不会获得 Simple 权限。
- 旧会话标题仅作为查找线索时不再误触发参考一致性 Hard；标题之后明确要求对齐参考的指令仍触发。新任务纪元清除旧参考验收状态，避免已退休目标污染后续反馈。
- Schema 34、execution profile v14 和 stable-skill schema 10 保持不变；执行与本次行为直接相关的定向测试和仓库校验后发布。
...
- 将 Hard 评估者与父级计划/复核指定为 `gpt-6-sol`、`ultra`，执行子任务指定为 `gpt-6-sol`、`medium`，并在宿主请求校验中精确绑定模型和推理档位；Simple 和 Daily 沿用用户在输入框选择的模型。
- Schema 34、stable-skill schema 10 保持不变；执行 profile 升至 v14。只执行本次行为与版本契约相关的定向验证，采用 forward-only 1.0.71 发布。
...
- difficulty classifier 升级到 v5；Schema 34、execution profile v13 与 stable-skill schema 10 保持不变。按最低验证范围执行三项定向 Hook 测试和一次仓库校验后 forward-only 发布。
